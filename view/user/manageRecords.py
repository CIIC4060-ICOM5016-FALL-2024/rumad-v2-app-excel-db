...
class ManageRecords:
    ...
    tables = {
        ...
        "meeting": [
            ...
            ("starttime", "time"),
            ("endtime", "time"),
            ("cdays", ["MWF", "TTH"]),  # Predefined Options for Days
        ],
        "requisite": [
            ("classid", "search", "cid", "cname", None),
            ("reqid", "search", "cid", "cname", None),
            ("prereq", bool),
        ],
        "room": [
            ("building", str),
        ...
        if table_selected:
            ...
            if st.button("Delete Record"):
                ...
                if response.status_code == 200:
                    ...
                    st.error("Failed to delete record. Ensure the ID is correct.")
    ...
    def generate_form(self, table_selected, record=None):
        """
        Dynamically generate form fields based on table schema.
        """
        valid = True
        fields = {}
        for column in self.tables[table_selected]:
            ...
            if column_type == "search":
                response = requests.get(f"{self.mainRoute}{column_name}")
                options = {rec[column[3]]: rec[column[2]] for rec in response.json()}
                selected_option = st.selectbox(f"{column_name}", options.keys(), index=0 if current_value is None else list(options.values()).index(current_value))
                ...
                selected_time = st.time_input(f"{column_name}", time_value)
                fields[column_name] = selected_time.strftime("%H:%M:%S")

            elif type(column_type) == list:
                selected_value = st.selectbox(f"{column_name}", column_type, index=0 if current_value is None else column_type.index(current_value))
                fields[column_name] = selected_value

            elif column_type == str:
                entered_text = st.text_input(f"{column_name}", value=current_value or "")
                if not entered_text:
                    valid = False
                fields[column_name] = entered_text

            elif column_type == int:
                entered_number = st.number_input(f"{column_name}", value=current_value or 0, min_value=0)
                fields[column_name] = entered_number

            elif column_type == bool:
                is_checked = st.checkbox(f"{column_name}", value=current_value or False)
